...
namespace XcyUI.widgets
{
    public class XViewBuilder
    {
        ...
        {
            return new XViewBuilder(view, false);
        }
        ...
        public XViewBuilder(XView view, bool isCreated)
        {
            View = view; 
        ...
        public XViewBuilder ClipPadding(bool isClip)
        {
            View.Style.Also(n=>n.IsClipPadding = isClip);
            return this;
        }

        public XViewBuilder ClipContent(bool clip = true)
        {
            View.Style.Also(n => n.IsClipContent = clip);
            return this;
        }
        ...
        public XViewBuilder Binding<T>(XState<T> state, XFunction<XViewBuilder, T> function, bool needLayout = false, bool needParentLayout = false)
        {
            ...
            if (isCreated)
            {
                XFunction<T> observer = (t) =>
                {
                    ...
                    if (needLayout)
                    {
                        (View.GetColumnParent() ?? View).StartLayout();
                        View.Invalidate();
                    }
                    else if(needParentLayout)
                    {
                        var wrapParent = View.GetColumnParent() ??View.GetWrapParent();
                        var parent = wrapParent is XLazy ? View.Parent : View.Parent;
                        parent.StartLayout();
                        View.Invalidate();
                    }
