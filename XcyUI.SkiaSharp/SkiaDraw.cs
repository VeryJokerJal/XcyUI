...
namespace XcyUI.SkiaSharp
{
    public class SkiaDraw : IDraw
    {
        ...
        public unsafe SKBitmap BufferToSKBitmap(byte[] rgbaBuffer, int width, int height)
        {
            // 先创建 SKBitmap 并分配内部像素内存
            var bitmap = new SKBitmap(width, height, SKColorType.Rgba8888, SKAlphaType.Premul);

            fixed (byte* ptr = rgbaBuffer)
            {
                // 将像素数据安装到位图中
                // 使用带 rowBytes 的重载：每行字节数 = width * 4
                bool success = bitmap.InstallPixels(
                    new SKImageInfo(width, height, SKColorType.Rgba8888, SKAlphaType.Premul),
                    (IntPtr)ptr,
                    width * 4);

                if (!success)
                    throw new InvalidOperationException("安装像素失败");
            }
